...
{
  if (GET_POS(vict) < POS_LYING || GET_POS(ch) < POS_LYING)
    return;
...
ACMD(do_tell)
{
  ...
  if (GET_POS(vict) < POS_LYING || PRF_FLAGGED(vict, PRF_NOTELL))
...
ACMD(do_language)
{
  ...
  if ((lannum = find_skill_num(arg)) && (lannum >= SKILL_ENGLISH && lannum <= SKILL_GERMAN))
  ...
  if (subcmd == SCMD_ANSWER) {
    ...
    if (!cyber) {
      for (tch = world[ch->in_room].people; tch; tch = tch->next_in_room)
        if (tch != ch) {
          if (success_test(GET_SKILL(tch, GET_LANGUAGE(ch)), 4) > 0)
            act(buf2, FALSE, ch, 0, tch, TO_VICT | TO_SLEEP);
          else
