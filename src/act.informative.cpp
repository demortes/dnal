	...
	*buf = '\0';
	...
	if (mode == 7 || mode == 8)
		if (GET_OBJ_TYPE(object) == ITEM_HOLSTER)
		{
			...
			strcat(buf, " carrying:");
		else if (GET_OBJ_TYPE(object) == ITEM_FOCUS && GET_OBJ_VAL(object, 4))
...
void look_at_room(struct char_data * ch, int ignore_brief)
{
	...
	if (world[ch->in_room].poltergeist[0])
		send_to_char(ch, "^cAn invisible force is whiping small objects around the area.^n\r\n");
		...
		if ((GET_OBJ_TYPE(tmp_object) == ITEM_DRINKCON) ||
			...
			for (i = 7; i < 10; ++i)
				if (GET_OBJ_VAL(tmp_object, i) > 0) {
					sprintf(buf1, "There is %s attached to the %s of it.\r\n",
							short_object(GET_OBJ_VAL(tmp_object, i), 2), "side");
			...
			if (tmp_object->contains)
				sprintf(buf, "It contains %d round%s of %s ammunition, and can hold %d round%s.\r\n",
						...
						"s", ammo_type[GET_OBJ_VAL(tmp_object->contains, 2)],
						GET_OBJ_VAL(tmp_object, 5), "s");
			else
			...
			send_to_char(ch, "It has %d round%s left.\r\n", GET_OBJ_VAL(tmp_object, 9),
...
ACMD(do_pool)
{
	...
	if (GET_MAGIC(ch) > 0)
		sprintf(ENDOF(pools), "  Spell: %d (Casting: %d    Drain: %d)\r\n", GET_MAGIC(ch), GET_CASTING(ch), GET_DRAIN(ch));
	if (GET_CONTROL(ch) > 0)
...
ACMD(do_score)
{
	...
	if (AFF_FLAGGED(ch, AFF_RIG) || PLR_FLAGGED(ch, PLR_REMOTE)) {
		...
		if (GET_TRADITION(ch) != TRAD_MUNDANE)
			...
			strcpy(buf2, "");
		sprintf(buf, "%s^L/^b/ ^nArmor     ^w[ ^W%2d^rB^w/ ^W%2d^rI^w]    %-11s"
				"                              ^b/^L/\r\n", buf,
				GET_BALLISTIC(ch), GET_IMPACT(ch), buf2);

		sprintf(buf, "%s^b/^L/ ^nNuyen     ^w[^W%'9d^w]               "
				"                              ^L/^b/\r\n", buf,
				GET_NUYEN(ch));
	...
	for (; sort != 0; sort > 0 ? sort-- : sort++) {
		...
		for (d = descriptor_list; d; d = d->next) {
			...
			if (GET_LEVEL(ch) > LVL_MORTAL) {
				...
				if (PRF_FLAGGED(tch, PRF_NEWBIEHELPER))
...
ACMD(do_status)
{
	...
	if (GET_DRUG_AFFECT(ch) && GET_DRUG_STAGE(ch) > 0)
		send_to_char(ch, "  %s (%s)\r\n", drug_types[GET_DRUG_AFFECT(ch)].name, GET_DRUG_STAGE(ch) == 1 ? "Up" : "Down");
