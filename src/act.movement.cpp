...
int can_move(struct char_data *ch, int dir, int extra)
{
  ...
  if (IS_WATER(ch->in_room) && !IS_NPC(ch) &&
      !IS_SENATOR(ch))
  {
    target = MAX(2, (int) world[ch->in_room].rating);
...
void move_vehicle(struct char_data *ch, int dir)
{
  ...
  if(!EXIT(veh, dir) || EXIT(veh, dir)->to_room == NOWHERE ||
      (!ROOM_FLAGGED(EXIT(veh, dir)->to_room, ROOM_ROAD) && !ROOM_FLAGGED(EXIT(veh,
          dir)->to_room, ROOM_GARAGE)) && (veh->type != VEH_DRONE && veh->type != VEH_BIKE) ||
  ...
  for (v = veh->followers; v; v = nextv)
  {
    ...
    if (get_speed(v->follower) > 80 && SECT(v->follower->in_room) == SPIRIT_CITY || world[v->follower->in_room].icesheet[0]) {
      crash_test(pilot);
      chkdmg(v->follower);
    }
  }
