...
ACMD(do_retract)
{
  struct obj_data *cyber = ch->cyberware;
  for (;cyber; cyber = cyber->next_content)
    if (GET_OBJ_VAL(cyber, 2) == 19 || GET_OBJ_VAL(cyber, 2) == 21)
      break;
  if (!cyber)
    send_to_char("You don't have any retractable cyberweapons.\r\n", ch);
  else {
    if (GET_OBJ_VAL(cyber, 9)) {
      GET_OBJ_VAL(cyber, 9)--;
      sprintf(buf, "$n retracts a pair of %s.", GET_OBJ_NAME(cyber)+2);
      act(buf, TRUE, ch, 0, 0, TO_ROOM);
      send_to_char(ch, "You retract a pair of %s.\r\n", GET_OBJ_NAME(cyber)+2);
    } else {
      GET_OBJ_VAL(cyber, 9)++;
      sprintf(buf, "$n extends a pair of %s.", GET_OBJ_NAME(cyber)+2);
      act(buf, TRUE, ch, 0, 0, TO_ROOM);
      send_to_char(ch, "You extend a pair of %s.\r\n", GET_OBJ_NAME(cyber)+2);
    }
    affect_total(ch);
  }
}
