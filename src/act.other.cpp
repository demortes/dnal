...
ACMD(do_patch)
{
  ...
  case 1:                          // stim
    ...
    GET_MENTAL(ch) = MIN((int) GET_MAX_MENTAL(ch), (int)GET_MENTAL(ch) + (int)(GET_OBJ_VAL(patch, 1) * 100));
    obj_from_char(patch);
    GET_EQ(vict, WEAR_PATCH) = patch;
    patch->worn_by = vict;
    patch->worn_on = WEAR_PATCH;
...
ACMD(do_use)
{
  ...
  if (GET_OBJ_TYPE(obj) == ITEM_MONEY && GET_OBJ_VAL(obj, 1)) {
    ...
    if (GET_DRUG_DOSE(ch) > GET_DRUG_TOLERANT(ch, drugval)) {
      ...
      case DRUG_CRAM:
      case DRUG_BURN:
        GET_DRUG_DURATION(ch) = 10;
        break;
      case DRUG_ZEN:
        GET_DRUG_DURATION(ch) = 5 * srdice();
      default:
      ...
      if (GET_POWER(ch, i) > 0) {
        ...
        if (max_ability(i) > 1)
          ...
          case ADEPT_KILLING_HANDS:
            sprintf(buf2 + strlen(buf2), " %-8s\r\n", wound_name[MIN(4, (int) GET_POWER(ch, i))]);
            break;
          default:
...
ACMD(do_defense)
{
  ...
  if (GET_BODY(ch)) {
    GET_BODY(ch) = MAX(0, MIN((int) GET_BODY(ch), GET_COMBAT(ch) - GET_DEFENSE(ch) - GET_OFFENSE(ch)));
    ch->real_abils.body_pool = GET_BODY(ch);
  }
...
ACMD(do_body)
{
  skip_spaces(&argument);
  int i = atoi(argument);
  if (i < 0 || i > GET_DEFENSE(ch) + GET_BODY(ch))
    send_to_char(ch, "Body pool must be between 0 and %d.\r\n", GET_DEFENSE(ch) + GET_BODY(ch));
  else {
    GET_DEFENSE(ch) = GET_COMBAT(ch) - GET_OFFENSE(ch);
    GET_BODY(ch) = i;
    ch->real_abils.body_pool = GET_BODY(ch);
    GET_DEFENSE(ch) -= GET_BODY(ch);
    ch->real_abils.defense_pool = GET_DEFENSE(ch);
    send_to_char(ch, "Your body pool is now at %d, defense is now at %d.\r\n", GET_BODY(ch), GET_DEFENSE(ch));
  }
}
...
ACMD(do_treat)
{
  ...
  if (success_test(i, target)) {
    ...
    if (GET_PHYSICAL(vict) < 100) {
      GET_PHYSICAL(vict) = MIN((int) GET_MAX_PHYSICAL(vict), 100);
      GET_MENTAL(vict) = 0;
...
ACMD(do_astral)
{
  ...
  GET_REAL_INT(astral) = GET_REAL_INT(ch);
  GET_INT(astral) = GET_REAL_INT(ch);
  GET_REAL_WIL(astral) = GET_REAL_WIL(ch);
  GET_WIL(astral) = GET_REAL_WIL(ch);
  ...
  if (vict) {
    ...
    if (!obj) {
      ...
      if (success < 3) {
        ...
        } else if (GET_TRADITION(vict) != TRAD_MUNDANE)
          strcat(buf, " is awakened");
        else
          strcat(buf, " is mundane");
        strcat(buf, ".\r\n");
        ...
        if (IS_NPC(vict)) {
          ...
          sprintf(ENDOF(buf), " and %d magic", (int)(GET_MAG(vict) / 100));
        else
          strcat(buf, " and is mundane");
        if (vict->cyberware) {
          strcat(buf, ". ");
    ...
    if (GET_OBJ_TYPE(obj) == ITEM_FOCUS) {
      ...
      if (success >= 5) {
        ...
        if (GET_OBJ_VAL(obj, 1) < GET_MAG(ch) / 100)
          ...
          strcat(buf, ". It has the same ammount of astral presence as you");
        else
      ...
      sprintf(ENDOF(buf), "is dedicated to %s", spirits[GET_OBJ_VAL(obj, 2)].name);
      if (success >= 5)
        sprintf(ENDOF(buf), ". It is of force %d", GET_OBJ_VAL(obj, 1));
      else if (success >= 3) {
        if (GET_OBJ_VAL(obj, 1) < GET_MAG(ch) / 100)
...
ACMD(do_unpack)
{
  ...
  if (ch->in_veh && !ch->in_veh->flags.IsSet(VFLAG_WORKSHOP)) {
    send_to_char(ch, "This vehicle isn't large enough to set up a workshop in.\r\n");
    return;
  }
  ...
  if (!shop)
    ...
    act("$n beings to set up $p.", FALSE, ch, 0, shop, TO_ROOM);
...
ACMD(do_packup)
{
  ...
  if (!shop)
    ...
    act("$n beings to pack up $p.", FALSE, ch, 0, shop, TO_ROOM);
    GET_OBJ_VAL(shop, 3) = 3;
    AFF_FLAGS(ch).SetBit(AFF_PACKING);
  }
}
