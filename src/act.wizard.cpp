...
ACMD(do_copyover)
{
  ...
  char *messages[] =
    {
      ...
      "Ahh drek, Che's broke the mud again!  Go bug Serge and he might fix it.\r\n",
      "Ahh drek, Serge's broke the mud again!  Go bug Che and he might fix it.\r\n",
  ...
  sprintf (buf, messages[number(0, 17)]);
...
ACMD(do_olcon)
{
  ...
  sprintf(buf, "OLC turned %s by %s.", (olc_state ? "ON" : "OFF"),
          GET_CHAR_NAME(ch));
  mudlog(buf, ch, LOG_WIZLOG, TRUE);
}
...
ACMD(do_vteleport)
{
  ...
  if (!*buf)
    ...
    sprintf(buf2, "%s teleported %s to %s", GET_CHAR_NAME(ch), veh->short_description, world[target].name);
    mudlog(buf2, ch, LOG_WIZLOG, TRUE);
...
{
  send_to_char(ch, "Name: '^y%s^n', Keywords: %s\r\n", host->name, host->keywords);
  send_to_char(ch, "Vnum: [^g%5ld^n] Rnum: [%5ld] Parent: [%5ld]\r\n",
               host->vnum, real_host(host->vnum), host->parent);
  send_to_char(ch, "Type: [%10s] Security: [%s-%d^n] Alert: [%6s]\r\n", host_type[host->type],
               host_sec[host->colour], host->security, alerts[host->alert]);
  send_to_char(ch, "Shutdown: [%2d] Successes: [%2d] MPCP: [%2d]\r\n", host->shutdown,
               host->shutdown_success, host->shutdown_mpcp);
  send_to_char(ch, "Access: [^B%2d^n] Control: [^B%2d^n] Index: [^B%2d^n] Files: [^B%2d^n] Slave: [^B%2d^n]\r\n",
               host->stats[ACCESS][0], host->stats[CONTROL][0], host->stats[INDEX][0], host->stats[FILES][0], host->stats[SLAVE][0]);
  send_to_char(ch, "Deckers Present: ^y");
  bool prev = FALSE;
  for (struct matrix_icon *icon = host->icons; icon; icon = icon->next_in_host)
    if (icon->decker && icon->decker->ch)
    {
      send_to_char(ch, "%s%s (%d)", prev ? ", " : "", GET_CHAR_NAME(icon->decker->ch), icon->decker->tally);
      prev = TRUE;
    }
  send_to_char(ch, "^n\r\n");
}
...
void do_stat_veh(struct char_data *ch, struct veh_data * k)
{
  ...
  sprintf(buf, "Vnum: [^g%5ld^n] Rnum: [%5ld] Type: [%10s]\r\n",
          virt, k->veh_number, veh_type[k->type]);
  send_to_char(buf, ch);
  sprintf(buf, "Han: [^B%d^n]  Spe: [^B%d^n]  Acc: [^B%d^n]  Bod: [^B%d^n]  Arm: [^B%d^n]\r\n",
          k->handling, k->speed, k->accel, k->body, k->armor);
  send_to_char(buf, ch);
  sprintf(buf, "Sig: [^B%d^n]  Aut: [^B%d^n]  Pil: [^B%d^n]  Sea: [^B%d^n]  Loa: [^B%d^n]  Cos: [^B%d^n]\r\n",
          k->sig, k->autonav, k->pilot, k->seating, (int)k->load, k->cost);
  send_to_char(buf, ch);
}
...
void do_stat_object(struct char_data * ch, struct obj_data * j)
{
  ...
  if (GET_OBJ_RNUM(j) >= 0)
  {
    ...
    strcpy(buf2, "None");
  sprintf(buf, "VNum: [^g%5ld^n], RNum: [%5ld], Type: %s, SpecProc: %s\r\n",
  ...
  j->obj_flags.wear_flags.PrintBits(buf, MAX_STRING_LENGTH,
                                    wear_bits, NUM_WEARS);
  strcat(buf, "\r\n");
  send_to_char(buf, ch);

  send_to_char("Set char bits : ", ch);
  j->obj_flags.bitvector.PrintBits(buf, MAX_STRING_LENGTH,
                                   affected_bits, AFF_MAX);
  strcat(buf, "\r\n");
  send_to_char(buf, ch);

  send_to_char("Extra flags   : ", ch);
  GET_OBJ_EXTRA(j).PrintBits(buf, MAX_STRING_LENGTH,
                             extra_bits, ITEM_EXTRA_MAX);
  strcat(buf, "\r\n");
  send_to_char(buf, ch);

  sprintf(buf, "Weight: %.2f, Value: %d, Timer: %d, Availability: %d/%.2f Days\r\n",
  ...
  send_to_char(ch, "Material: %s, Rating: %d, Condition: %d\r\n",
  ...
  case ITEM_FIREWEAPON:
    sprintf(buf, "Str Min: %d, Str+: %d, Power: %d, Wound: %s, Skill: %s, Type: %s\r\n",
    ...
    break;
  case ITEM_WEAPON:
    sprintf(buf, "Power: %d, Wound: %s, Str+: %d, DamType: %s, Skill: %s\r\nMax Ammo: %d, Ammo: %d",
            GET_OBJ_VAL(j, 0), wound_arr[GET_OBJ_VAL(j, 1)], GET_OBJ_VAL(j, 2),
            attack_types[GET_OBJ_VAL(j, 3) - TYPE_HIT],
            skills[GET_OBJ_VAL(j, 4)].name, GET_OBJ_VAL(j, 5), GET_OBJ_VAL(j, 6));
    if (GET_OBJ_VAL(j, 3) >= TYPE_PISTOL) {
      sprintf(buf2, ", Top: %d, Barrel: %d, Under: %d", GET_OBJ_VAL(j, 7), GET_OBJ_VAL(j, 8), GET_OBJ_VAL(j, 9));
      strcat(buf, buf2);
    } else if (GET_OBJ_VAL(j, 3) > TYPE_HIT && GET_OBJ_VAL(j, 3) < TYPE_TASER) {
      sprintf(buf2, ", Focus: %d, Rating: %d, By: %d", GET_OBJ_VAL(j, 7), GET_OBJ_VAL(j, 8), GET_OBJ_VAL(j, 9));
      strcat(buf, buf2);
    }
    break;
  case ITEM_MISSILE:
    sprintf(buf, "Missile type: %s", (GET_OBJ_VAL(j, 0) == 0 ? "Arrow" : "Bolt"));
    break;
  case ITEM_WORN:
    sprintf(buf, "Ballistic: %d, Impact: %d", GET_OBJ_VAL(j, 5), GET_OBJ_VAL(j, 6));
    break;
  case ITEM_DOCWAGON:
    sprintf(buf, "Contract: %s, Owner: %d", (GET_OBJ_VAL(j, 0) == 1 ? "basic" :
            (GET_OBJ_VAL(j, 0) == 2 ? "gold" : "platinum")), GET_OBJ_VAL(j, 1));
    break;
  case ITEM_CONTAINER:
    sprintf(buf, "Max-contains: %d, Locktype: %d, Key: %d, Lock level: %d, Corpse: %s",
            GET_OBJ_VAL(j, 0), GET_OBJ_VAL(j, 1), GET_OBJ_VAL(j, 2),
  ...
  case ITEM_MONEY:
    if (!GET_OBJ_VAL(j, 1))
      sprintf(buf, "Value: %d, Type: nuyen", GET_OBJ_VAL(j, 0));
    else
      sprintf(buf, "Value: %d, Type: credstick, Security: %s", GET_OBJ_VAL(j, 0),
  ...
  case ITEM_KEY:
    ...
    break;
  case ITEM_FOOD:
    sprintf(buf, "Makes full: %d, Poisoned: %d", GET_OBJ_VAL(j, 0), GET_OBJ_VAL(j, 3));
    break;
  case ITEM_QUIVER:
    sprintf(buf, "Max-contains: %d, Type contains: %s", GET_OBJ_VAL(j, 0), (GET_OBJ_VAL(j, 1) == 0 ? "Arrow" :
            (GET_OBJ_VAL(j, 1) == 1 ? "Bolt" : (GET_OBJ_VAL(j, 1) == 2 ? "Shuriken" :
                                                (GET_OBJ_VAL(j, 1) == 3 ? "Throwing knife" : "Undefined")))));
    break;
  case ITEM_PATCH:
    sprintf(buf, "Type: %s, Rating: %d", patch_names[GET_OBJ_VAL(j, 0)],
            GET_OBJ_VAL(j, 1));
    break;
  case ITEM_CYBERDECK:
    ...
    break;
  case ITEM_PROGRAM:
    if (GET_PROG_TYPE(j) == SOFT_ATTACK)
      sprintf(buf2, " DamType: %s, ", wound_name[GET_OBJ_VAL(j, 3)]);
    else
      sprintf(buf2, " ");
    sprintf(buf, "Type: %s, Rating: %d, Size: %d,%sInstalled?: %s",
            programs[GET_OBJ_VAL(j, 0)].name, GET_OBJ_VAL(j, 1), GET_OBJ_VAL(j, 2), buf2,
            (GET_OBJ_VAL(j, 6) ? "yes" : "no"));
    break;
  case ITEM_CYBERWARE:
    sprintf(buf, "Rating: %d, Essence: %d, Type: %s", GET_OBJ_VAL(j, 0),
            GET_OBJ_VAL(j, 1), cyberware_names[GET_OBJ_VAL(j, 2)]);
    break;
  case ITEM_WORKSHOP:
sprintf(buf, "Type: %s, For: %s", GET_OBJ_VAL(j, 1) ? GET_OBJ_VAL(j, 1) == 3 ? "Facility": "Workshop" : "Kit", workshops[GET_OBJ_VAL(j, 0)]);
    break;
  case ITEM_FOCUS:
    sprintf(buf, "Type: %s Force: %d", foci_type[GET_OBJ_VAL(j, 0)], GET_OBJ_VAL(j, 1));
    break;
  case ITEM_SPELL_FORMULA:
    sprintf(buf, "Type: %s Force: %d Tradition: %s", spells[GET_OBJ_VAL(j, 1)].name, GET_OBJ_VAL(j, 0), GET_OBJ_VAL(j, 2) ? "Shamanic" : "Hermetic");
  default:
    break;
  }
  send_to_char(buf, ch);

  sprintf(buf, "\r\nValues 0-9: [%d] [%d] [%d] [%d] [%d] [%d] [%d] [%d] [%d] [%d]",
          GET_OBJ_VAL(j, 0), GET_OBJ_VAL(j, 1),
  ...
  if (j->contains)
  {
    ...
    for (found = 0, j2 = j->contains; j2; j2 = j2->next_content) {
      sprintf(buf2, "%s %s", found++ ? "," : "", GET_OBJ_NAME(j2));
      strcat(buf, buf2);
      if (strlen(buf) >= 62) {
        if (j2->next_content)
          send_to_char(strcat(buf, ",\r\n"), ch);
...
void do_stat_character(struct char_data * ch, struct char_data * k)
{
  ...
  sprintf(buf2, " PC '%s'  IDNum: [%5ld], In room [%5ld]\r\n",
  ...
  {
  case TRAD_ADEPT:
    sprintf(buf, "Tradition: Adept, Grade: %d Extra Power: %d/%d", GET_GRADE(k), k->points.extrapp, (int)(GET_REP(k) / 50) + 1);
    break;
  case TRAD_HERMETIC:
    sprintf(buf, "Tradition: Hermetic, Aspect: %s, Grade: %d", aspects[GET_ASPECT(k)], GET_GRADE(k));
    break;
  case TRAD_SHAMANIC:
    sprinttype(GET_TOTEM(k), totem_types, buf2);
    sprintf(buf, "Tradition: Shamanic, Aspect: %s, Totem: %s, ", aspects[GET_ASPECT(k)], buf2);
  ...
  sprintf(buf, "Last room: [%ld], Start room: [%ld], Speaking: [%d]\r\n",
          k->player.last_room, GET_LOADROOM(k), GET_LANGUAGE(k));
  send_to_char(buf, ch);

  if (GET_QUEST(k))
    send_to_char(ch, "Quest: [%ld]\r\n", quest_table[GET_QUEST(k)].vnum);

  sprintf(buf, "Bod: [^c%d^n]  Qui: [^c%d^n]  Str: [^c%d^n]  Cha: [^c%d^n] Int: [^c%d^n]"
          "  Wil: [^c%d^n]  Mag: [^c%d^n]\r\nRea: [^c%d^n]  Ess: [^c%0.2f^n] Ast[^c%d^n]  Com[^c%d^n]  Mag[^c%d^n]  Hak[^c%d^n] Def/Off[^c%d/%d^n]\r\n",
          GET_BOD(k), GET_QUI(k), GET_STR(k), GET_CHA(k), GET_INT(k),
          GET_WIL(k), ((int)GET_MAG(k) / 100), GET_REA(k), ((float)GET_ESS(k) / 100), GET_ASTRAL(k),
          GET_COMBAT(k), GET_MAGIC(k), GET_HACKING(k), GET_DEFENSE(k), GET_OFFENSE(k));
  send_to_char(buf, ch);

  if ( !IS_NPC(k) )
  {
    sprintf(buf, "Bod: [^B%d^n]  Qui: [^B%d^n]  Str: [^B%d^n]  Cha: [^B%d^n] Int: [^B%d^n]"
  ...
  /* Changed Body Index to Essence Index as Per MM Washu*/
  /* Bio Index restored, and Essence Index added... */
  sprintf(buf, "Physical p.:[^G%d/%d^n]  Mental p.:[^G%d/%d^n]  Bio Index:[^c%0.2f^n]\r\n"
          "Ess Index:[^c%0.2f^n]\n\r",
          (int)(GET_PHYSICAL(k) / 100), (int)(GET_MAX_PHYSICAL(k) / 100),
          (int)(GET_MENTAL(k) / 100), (int)(GET_MAX_MENTAL(k) / 100),
          ((float)GET_INDEX(k) / 100), ((float)(GET_ESS(ch) / 100) + 3));
  send_to_char(buf, ch);

  sprintf(buf, "Nuyen: [%9d], Bank: [%9d] (Total: %d), Karma: [%0.2f]\r\n",
          GET_NUYEN(k), GET_BANK(k), GET_NUYEN(k) + GET_BANK(k),
          ((float)GET_KARMA(k) / 100));
  send_to_char(buf, ch);

  sprintf(buf, "B: %d (%d), I: %d (%d), I-Dice: %d, I-Roll: %d, Sus: %d, Foci: %d, TargMod: %d\r\n",
          GET_BALLISTIC(k), GET_TOTALBAL(k), GET_IMPACT(k), GET_TOTALIMP(k), GET_INIT_DICE(k), GET_INIT_ROLL(k),
          GET_SUSTAINED_NUM(k), GET_FOCI(k), GET_TARGET_MOD(k));
  send_to_char(buf, ch);
  sprintf(buf, "Current Vision: %s Natural Vision: %s\r\n",
  ...
  sprintf(buf, "Current Zone: %d, Pos: %s, Fighting: %s",
  ...
  sprintf(buf, "Carried: weight: %.2f (Max: %d), items: %d; ",IS_CARRYING_W(k), CAN_CARRY_W(k),IS_CARRYING_N(k));
...
void do_stat_mobile(struct char_data * ch, struct char_data * k)
{
  ...
  send_to_char(strcat(buf, buf2), ch);
  sprintf(buf, "Alias: %s, VNum: [%5ld], RNum: [%5ld]\r\n", GET_KEYWORDS(k),
  ...
  strcpy(buf, "Class: ");
  sprinttype(GET_RACE(k), npc_classes, buf2);
  strcat(buf, buf2);

  sprintf(buf, "Bod: [^c%d^n]  Qui: [^c%d^n]  Str: [^c%d^n]  Cha: [^c%d^n] Int: [^c%d^n]"
          "  Wil: [^c%d^n]  Mag: [^c%d^n]\r\nRea: [^c%d^n]  Ess: [^c%0.2f^n]  Ast[^c%d^n]  Com[^c%d^n]  Mag[^c%d^n]  Hak[^c%d^n]  Def/Off[^c%d/%d^n]\r\n",
          GET_BOD(k), GET_QUI(k), GET_STR(k), GET_CHA(k), GET_INT(k),
          GET_WIL(k), GET_MAG(k) / 100, GET_REA(k), ((float)GET_ESS(k) / 100), GET_ASTRAL(k),
          GET_COMBAT(k), GET_MAGIC(k), GET_HACKING(k), GET_DEFENSE(k), GET_OFFENSE(k));
  send_to_char(buf, ch);

  sprintf(buf, "Physical p.:[^G%d/%d^n]  Mental p.:[^G%d/%d^n]\r\n",
          (int)(GET_PHYSICAL(k) / 100), (int)(GET_MAX_PHYSICAL(k) / 100),
          (int)(GET_MENTAL(k) / 100), (int)(GET_MAX_MENTAL(k) / 100));
  send_to_char(buf, ch);
  ...
  sprintf(buf, "Nuyen: [%5d], Credstick: [%5d], Bonus karma: [%4d], Total karma: [%4d]\r\n",
          GET_NUYEN(k), GET_BANK(k), GET_KARMA(k), base);
  send_to_char(buf, ch);

  sprintf(buf, "B: %d, I: %d, I-Dice: %d, I-Roll: %d, Sus: %d, TargMod: %d\r\n",
          GET_BALLISTIC(k), GET_IMPACT(k), GET_INIT_DICE(k), GET_INIT_ROLL(k),
          GET_SUSTAINED_NUM(k), GET_TARGET_MOD(k));
  send_to_char(buf, ch);
  ...
  sprintf(buf, "NPC flags: ^c%s\r\n", buf2);
  send_to_char(buf, ch);

  sprintf(buf, "Height: %d cm, Weight: %d kg\n", GET_HEIGHT(k), GET_WEIGHT(k));
  send_to_char(buf, ch);

  sprintf(buf, "Carried: weight: %.2f, items: %d; ",
  ...
  if (IS_SENATOR(victim)) {
    ...
    if (PLR_FLAGS(victim).IsSet(PLR_NEWBIE)) {
      ...
      for (cyber = ch->cyberware; cyber; cyber = cyber->next_content)
        if (IS_OBJ_STAT(cyber, ITEM_STARTER))
          GET_OBJ_EXTRA(cyber).RemoveBit(ITEM_STARTER);
      for (obj = ch->carrying; obj; obj = obj->next_content) {
        for (cyber = obj->contains; cyber; cyber = cyber->next_content)
          if (IS_OBJ_STAT(cyber, ITEM_STARTER))
            GET_OBJ_EXTRA(cyber).RemoveBit(ITEM_STARTER);
        if (IS_OBJ_STAT(obj, ITEM_STARTER))
          GET_OBJ_EXTRA(obj).RemoveBit(ITEM_STARTER);
      }
      for (i = 0; i < (NUM_WEARS - 1); i++)
        if (GET_EQ(ch, i)) {
          for (obj = GET_EQ(ch, i)->contains; obj; obj = obj->next_content)
            if (IS_OBJ_STAT(obj, ITEM_STARTER))
              GET_OBJ_EXTRA(obj).RemoveBit(ITEM_STARTER);
          if (IS_OBJ_STAT(GET_EQ(ch, i), ITEM_STARTER))
            GET_OBJ_EXTRA(GET_EQ(ch, i)).RemoveBit(ITEM_STARTER);
        }
    }
...
ACMD(do_dc)
{
  ...
  if (*arg == '*') {
    ...
    mudlog(buf, ch, LOG_WIZLOG, TRUE);
    for (d = descriptor_list; d; d = d->next)
      if ((d->connected > 0 && d->connected < CON_SPELL_CREATE) || d->connected > CON_DECK_CREATE)
        close_socket(d);
    return;
  }
...
ACMD(do_last)
{
  ...
  // if the player is on, we don't need to retrieve anything
  if ((vict = playerDB.GetChar(arg)) == NULL) {
    // otherwise, we have to load from the file
    vict = playerDB.LoadChar(arg, TRUE);
  ...
  if ((GET_LEVEL(vict) > GET_LEVEL(ch)) && !access_level(ch, LVL_ADMIN)) {
  ...
  if (subcmd == SCMD_FINGER)
    sprintf(buf, "%-12s :: Last logon:  %-20s\r\n",
            GET_NAME(vict), ctime(&vict->player.time.logon));
  else
    sprintf(buf, "[%5ld] [%2d] %-12s : %-18s : %-20s\r\n",
            GET_IDNUM(vict), (int) GET_LEVEL(vict),
            GET_NAME(vict), vict->player.host,
  ...
  if (IS_NPC(ch))
    ...
    if (strstr((const char *)argument, "^l")) {
      ...
      sprintf(buf, "Okay, you're now %s %s %s.\r\n",
              GET_PRETITLE(ch), GET_NAME(ch), GET_TITLE(ch));
      send_to_char(buf, ch);
    }
  }
}
...
ACMD(do_wizutil)
{
  ...
  if (!*arg)
    ...
    switch (subcmd) {
    case SCMD_POWERPOINT:
      if (GET_TRADITION(vict) != TRAD_ADEPT) {
        send_to_char(ch, "%s is not an adept.\r\n", GET_CHAR_NAME(vict));
        return;
      }
      if (GET_KARMA(vict) < 2000 || vict->points.extrapp > (int)(GET_REP(vict) / 50)) {
        send_to_char(ch, "%s does not have enough karma to purchase a powerpoint.\r\n", GET_CHAR_NAME(vict));
        return;
      }
      GET_KARMA(vict) -= 2000;
      GET_PP(vict) += 100;
      vict->points.extrapp++;
      sprintf(buf, "%s gave %s an extra powerpoint.", GET_CHAR_NAME(ch), GET_CHAR_NAME(vict));
      mudlog(buf, ch, LOG_WIZLOG, TRUE);
      send_to_char(vict, "You have purchased an additional powerpoint.\r\n");
      send_to_char(ch, "%s given an extra powerpoint.\r\n", GET_CHAR_NAME(vict));
      break;
    case SCMD_INITIATE:
      if (GET_TRADITION(vict) == TRAD_MUNDANE) {
        send_to_char(ch, "%s is mundane.\r\n", GET_CHAR_NAME(vict));
        return;
      }
      karmacost = (GET_GRADE(vict) + 6) * 300;
      if (karmacost > GET_KARMA(vict)) {
        send_to_char(ch, "%s does not have enough karma to initiate\n\r", GET_CHAR_NAME(vict));
        return;
      }
      GET_KARMA(vict) -= karmacost;
      GET_GRADE(vict)++;
      vict->real_abils.rmag += 100;
      if (GET_TRADITION(vict) == TRAD_ADEPT)
        GET_PP(vict) += 100;
      vict->real_abils.mag += 100;
      playerDB.SaveChar(vict);
      send_to_char(vict, "You have reached initiate grade %d\n\r", GET_GRADE(vict));
      send_to_char(ch, "%s has been initiated to grade %d\r\n", GET_CHAR_NAME(vict), GET_GRADE(vict));
      sprintf(buf, "%s has been initiated to grade %d by %s", GET_CHAR_NAME(vict), GET_GRADE(vict), GET_CHAR_NAME(ch));
      mudlog(buf,ch,LOG_WIZLOG,TRUE);
      break;
    case SCMD_PARDON:
    ...
    case SCMD_SQUELCHOOC:
      ...
      break;
    case SCMD_PG:
      result = PLR_TOG_CHK(vict, PLR_PG);
      sprintf(buf, "PG toggled %s for %s by %s.", ONOFF(result),
              GET_CHAR_NAME(vict), GET_CHAR_NAME(ch));
      if (PLR_FLAGGED(vict, PLR_PG)) {
        send_to_char(vict, "^RYou have just been labeled a power-gamer.\r\nYour greed has been your downfall.\r\n");
        send_to_char(ch, "Your target is now considered a powergamer.\r\n");
      } else {
        send_to_char(vict, "^RCongratulations, you're no longer considered a powergamer.\r\n");
        send_to_char(ch, "Your target is no longer considered a powergamer.\r\n");
      }
      mudlog(buf, ch, LOG_WIZLOG, TRUE);
      break;
    case SCMD_RPE:
...
ACMD(do_show)
{
  ...
  case 2:                     /* player */
...

    {
      ...
      if ((vict = playerDB.GetChar(value)) == NULL) {
        vict = playerDB.LoadChar(value, TRUE);
      ...
      sprintf(buf, "%s¥: %-8d  Bal: %-8d  Karma: %-8d\r\n",
      ...
      sprintf(buf,
              ...
              buf, birth, ctime(&vict->player.time.logon),
...
#define GOD_ROOMS_ZONE 0
  ...
  case 11:
    strcpy(buf, "Lodges\r\n------\r\n");
    for (i = 0, j = 0; i <= top_of_world; i++)
      if (ROOM_FLAGGED(i, ROOM_MEDICINE_LODGE))
        sprintf(buf, "%s%2d: [%5ld] %s (%2d) %s\r\n", buf, ++j,
                world[i].number,
                from_ip_zone(world[i].number) ? " " : "*",
                world[i].rating, world[i].name);
    send_to_char(buf, ch);
    break;
  case 12:
    strcpy(buf, "Libraries\r\n---------\r\n");
    for (i = 0, j = 0; i <= top_of_world; i++)
      if (ROOM_FLAGGED(i, ROOM_HERMETIC_LIBRARY))
        sprintf(buf, "%s%2d: [%5ld] %s (%d) %s\r\n", buf, ++j,
                world[i].number,
                from_ip_zone(world[i].number) ? " " : "*",
                world[i].rating, world[i].name);
    send_to_char(buf, ch);
      ...
      if (GET_POWER(vict, i) > 0) {
        ...
        if (max_ability(i) > 1)
          ...
          case ADEPT_KILLING_HANDS:
            sprintf(buf2 + strlen(buf2), " %-8s\r\n", wound_name[MIN(4, GET_POWER(vict, i))]);
            break;
          default:
...
{ if (vict && is_file && !IS_NPC(vict)) { \
    if ((save)) playerDB.Update(vict); \
    playerDB.StoreChar(vict, (save)); \
  } \
}
...
ACMD(do_vset)
{
  ...
  if (is_abbrev(field, "owner")) {
    ...
    value--;
    veh->locked = value;
    sprintf(buf, "%s's lock was set to %s", veh->short_description, (value) ? "ON" : "OFF");
    ...
    veh->sub = value;
    sprintf(buf, "%s's subscribed status was set to %s", veh->short_description, (value) ? "ON" : "OFF");
  }
  send_to_char(buf, ch);
  return;
}
ACMD(do_set)
{
  ...
  case 39:
    ...
    if ((IS_SENATOR(vict) && !access_level(vict, LVL_ADMIN))) {
  ...
  case 61:
    ...
    break;
  case 62:
    SET_OR_REMOVE(PLR_FLAGS(vict), PLR_PG);
    break;
  case 63:
  ...
  if (is_file) {
    if (vict && is_file && !IS_NPC(vict)) {
      playerDB.StoreChar(vict, TRUE);
      playerDB.Update(vict);
    }
...
ACMD(do_logwatch)
{
  ...
  } else if (is_abbrev(buf, "deathlog") && access_level(ch, LVL_ADMIN) ) {
...
ACMD(do_unoperate)
{
  ...
  if (!*buf1) {
    send_to_char("Who do you want to unoperate on?\r\n", ch);
    return;
  }
  ...
  if (!*buf2) {
    send_to_char("You have to surgically remove something!\r\n", ch);
    return;
  } else if (!(cyber = get_obj_in_list_vis(vict, buf2, vict->cyberware)))
    if (!(cyber = get_obj_in_list_vis(vict, buf2, vict->bioware))) {
      act("$N doesn't seem to have that.", FALSE, ch, 0, vict, TO_CHAR);
      return;
    }

  if (GET_OBJ_TYPE(cyber) == ITEM_CYBERWARE)
    obj_from_cyberware(cyber);
  else
    obj_from_bioware(cyber);

  GET_OBJ_VAL(cyber, 4) = 0;

  act("You delicately remove $p from $N's body.", FALSE, ch, cyber, vict, TO_CHAR);
  act("$n performs a delicate procedure on $N.", FALSE, ch, 0, vict, TO_NOTVICT);
  act("$n delicately removes $p from your body.", FALSE, ch, cyber, vict, TO_VICT);

  obj_to_char(cyber, ch);
}
...
ACMD(do_zlist)
{
  ...
  int last_mob = 0, last_obj = 0, last_veh = 0;
...
ACMD(do_vlist)
{
  ...
  if ((first < 0) || (last < 0)) {
    ...
    send_to_char("The range cannot exceed 499 vehicles at a time.\r\n", ch);
    return;
  }
...
ACMD(do_ban_name)
{
  ...
  sprintf(buf, "%s has banned the name %s",
          GET_CHAR_NAME(ch), argument);
  mudlog(buf, ch, LOG_WIZLOG, TRUE);
...
ACMD(do_restring)
{
  ...
  sprintf(buf2, "%s succesfully restrung.\r\n", obj->text.name);
