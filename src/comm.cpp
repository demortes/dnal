...
int perform_subst(struct descriptor_data *t, char *orig, char *subst)
{
  ...
  *(second++) = '\0';
  ...
  if (!(strpos = strstr((char *)orig, first)))
  {
    SEND_TO_Q("Invalid substitution.\r\n", t);
    return 1;
  }
...
{
  if (d->edit_obj) {
    if (d->connected == CON_PART_CREATE || (d->connected >= CON_PRO_CREATE && d->connected <= CON_DECK_CREATE))
...
void close_socket(struct descriptor_data *d)
{
  ...
  if (d->character)
  {
    ...
    if (d->connected == CON_PART_CREATE || d->connected == CON_PLAYING || (d->connected >= CON_SPELL_CREATE &&
      ...
      if (PLR_FLAGGED(d->character, PLR_MATRIX)) {
