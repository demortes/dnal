...
void parse_veh(File &fl, long virtual_nr)
{
  ...
  veh_proto[veh_nr].seating = data.GetInt("Seating", 0);
...
void parse_room(File &fl, long nr)
{
  ...
  room->vision[0] = data.GetInt("POINTS/Light", -1);
  if (room->vision[0] == -1) {
...
void parse_object(File &fl, long nr)
{
  ...
  if (GET_OBJ_TYPE(obj) == ITEM_PROGRAM) {
    ...
    if (GET_OBJ_VAL(obj, 1) >= 21 && GET_OBJ_VAL(obj, 1) <= 25) {
      ...
      case AMMO_APDS:
        ...
        GET_OBJ_WEIGHT(obj) *= .25;
        GET_OBJ_AVAILDAY(obj) = 14;
        GET_OBJ_AVAILTN(obj) = 14;
        break;
      case AMMO_EXPLOSIVE:
        GET_OBJ_COST(obj) *= 5;
        GET_OBJ_WEIGHT(obj) *= .75;
...
void parse_shop(File &fl, long virtual_nr)
{
  ...
  int num_fields = data.NumSubsections("SELLING");
  ...
  for (int x = 0; x < num_fields; x++) {
    ...
    sell->vnum = data.GetLong(field, 0);
...
void load_zones(File &fl)
{
  ...
  fl.GetLine(buf, 256);
  if ((ptr = strchr((char *)buf, '~')) != NULL)   /* take off the '~' if it's there */
    *ptr = '\0';
  Z.name = str_dup(buf);
    ...
    for( power = 21; power >= 0; power-- )
      for( strength = 5; strength >= 0; strength-- )
      {
        for (nr = 0; nr <= top_of_objt; nr++) {
          ...
          ++found;
          sprintf(ENDOF(buf), "[%5ld -%2d] %2d%s +%d %s%s\r\n",
                  OBJ_VNUM_RNUM(nr),
                  ObjList.CountObj(nr),
...
void boot_shop_orders(void)
{
  ...
  for (int i = 0; i < top_of_shopt; i++) {
    ...
    for (int x = 0; x < num_ord; x++) {
      ...
      order->item = data.GetLong(buf, 0);
      sprintf(buf, "%s/Player", sect_name);
      order->player = data.GetLong(buf, 0);
