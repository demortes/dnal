...
void partbuild_main_menu(struct descriptor_data *d)
{
  ...
  send_to_char(CH, "2) Type: ^c%-40s^n     Parts Cost: ^c%d¥^n\r\n",
               parts[GET_OBJ_VAL(PART, 0)].name, get_part_cost(GET_OBJ_VAL(PART, 0), GET_OBJ_VAL(PART, 1), GET_OBJ_VAL(PART, 2)));
  send_to_char(CH, "3) MPCP Designed For: ^c%-30d^n  Chips Cost: ^c%d¥^n\r\n",
               GET_OBJ_VAL(PART, 2),
...
{
  if (ch->in_veh) {
    send_to_char(ch, "You can't cook a chip in a vehicle.\r\n");
    return;
  }
  ...
  for (struct obj_data *comp = world[ch->in_room].contents; comp; comp = comp->next_content)
  ...
  if (!chip)
    ...
    case SOFT_EVASION:
      break;
    default:
      send_to_char(ch, "That program doesn't need cooked.\r\n");
      return;
    }
    for (cooker = world[ch->in_room].contents; cooker; cooker = cooker->next_content)
      if (GET_OBJ_TYPE(cooker) == ITEM_DECK_ACCESSORY && GET_OBJ_VAL(cooker, 0) == TYPE_COOKER)
        break;
    if (!cooker) {
      send_to_char(ch, "There isn't a chip encoder here.\r\n");
      return;
    }
    if (cooker->contains) {
      send_to_char(ch, "There is already a chip in there.\r\n");
      return;
    }
...
ACMD(do_build)
{
  ...
  if (!*argument) {
    send_to_char(ch, "What part do you want to build?\r\n");
    return;
  }
  ...
  if (IS_WORKING(ch))
    send_to_char(ch, "You're too busy to build a part right now.\r\n");
  else if (GET_OBJ_TYPE(obj) != ITEM_PART || GET_OBJ_TYPE(deck) != ITEM_CUSTOM_DECK)
  ...
  else if ((GET_OBJ_VAL(obj, 2) != GET_OBJ_VAL(deck, 0) && GET_OBJ_VAL(deck, 0)) && (parts[GET_OBJ_VAL(obj, 0)].design >= 0 ||
           GET_OBJ_VAL(obj, 0) == PART_ACTIVE || GET_OBJ_VAL(obj, 0) == PART_STORAGE ||
           GET_OBJ_VAL(obj, 0) == PART_MATRIX_INTERFACE || GET_OBJ_VAL(obj, 0) == PART_MPCP))
    ...
    if (GET_OBJ_VAL(obj, 4) > 0) {
      ...
      GET_OBJ_VAL(obj, 4) = duration / success;
