    ...
    if (GET_OBJ_VAL(weapon, i) > 0
      ...
      for (obj = ch->cyberware; !mod && obj; obj = obj->next_content)
        if (GET_OBJ_VAL(obj, 2) == 22)
          if (GET_OBJ_VAL(obj, 0) == 1 || GET_OBJ_VAL(access, 2) < 2)
            mod = 2;
          else
            mod = 4;
      access = NULL;
...
void hit(struct char_data * ch, struct char_data * victim, int type)
{
  ...
  int tdistance=0, modtarget=0, tsight=0, total=0;
  ...
  if (IS_NERVE(ch) && !(GET_EQ(ch, WEAR_WIELD) || GET_EQ(ch, WEAR_HOLD)))
  {
    total = 4 + GET_IMPACT(victim);
    skill_total = get_skill(ch, SKILL_UNARMED_COMBAT, total);
    attack_success = success_test(skill_total, total);
    if (GET_QUI(victim) <= 0)
      attack_success = 0;
    if (attack_success > 0) {
      GET_TEMP_QUI_LOSS(victim) = MIN((GET_QUI(victim) * 4) + GET_TEMP_QUI_LOSS(victim), GET_TEMP_QUI_LOSS(victim) + (attack_success * 2));
      affect_total(victim);
      if (GET_QUI(victim) <= 0) {
  ...
  for (pos = WEAR_WIELD; !dead && pos <= WEAR_HOLD; pos++)
  {
    ...
    if (w_type != TYPE_HIT && w_type != TYPE_PUNCH && wielded) {
      ...
      if ((IS_NPC(ch) || GET_TRADITION(ch) == TRAD_ADEPT) &&
        ...
        for (obj = ch->cyberware;
          ...
          if (w_type == TYPE_HIT || w_type == TYPE_MELEE && GET_OBJ_VAL(obj, 9) == 1)
    ...
    if (!(IS_SPIRIT(victim) || IS_ELEMENTAL(victim)) && !PLR_FLAGGED(ch, PLR_REMOTE))
    ...
    if (IS_SPIRIT(victim) || IS_ELEMENTAL(victim))
      attack_resist = success_test(GET_WIL(victim), GET_CHA(ch) - GET_LEVEL(victim));
    else {
      if (GET_TOTALBAL(victim) > GET_QUI(victim))
        total += GET_TOTALBAL(victim) - GET_QUI(victim);
      else if (GET_TOTALIMP(victim) > GET_QUI(victim))
        total += GET_TOTALIMP(victim) - GET_QUI(victim);
      if (AWAKE(victim) && total < GET_QUI(victim))
        attack_resist = success_test(GET_DEFENSE(victim) + GET_DEFENSE(victim) ? GET_POWER(victim, ADEPT_SIDESTEP) : 0, 4 + damage_modifier(victim, buf) + (burst == 3 ? 1 : 0));
      else
    ...
    sprintf(rbuf,"Fight: I %2d, Ski %d, Targ %d, Succ %d. %sDef %d, Targ %d+%d, Succ %d. L:%d",
            ...
            AWAKE(victim) ? "" : "!A ",
            GET_DEFENSE(victim) + GET_DEFENSE(victim) ? GET_POWER(victim, ADEPT_SIDESTEP) : 0, power, modtarget, attack_resist,
            damage_total);
    ...
    if (IS_SPIRIT(ch) && !IS_NPC(victim) && (GET_TRADITION(victim) == TRAD_HERMETIC ||
      ...
      bod_success = success_test(MAX(GET_BOD(victim) + GET_BODY(victim), GET_SKILL(victim, SKILL_CONJURING)),
                                 power);
    else
      bod_success = success_test(GET_BOD(victim) + GET_BODY(victim), power);
    ...
    sprintf(rbuf, "Fight: Bod %d+%d, Pow %d, Suc %d.  %d(%d)->%d.  %d%c.",
            GET_BOD(victim), GET_BODY(victim), power, bod_success,
  ...
  if (wielded && GET_OBJ_VAL(wielded, 4) >= SKILL_MACHINE_GUNS && GET_OBJ_VAL(wielded, 4) <= SKILL_ASSAULT_CANNON)
  {
    for (int i = 0; i < (NUM_WEARS - 1); i++)
      if (GET_EQ(ch, i) && GET_OBJ_TYPE(GET_EQ(ch, i)) == ITEM_GYRO)
        return;
    damage(ch, ch, convert_damage(stage(-success_test(GET_BOD(ch) + GET_BODY(victim), power / 2 + modify_target(ch)), LIGHT))
           , TYPE_HIT, FALSE);
  }
}
...
void roll_initiative(void)
{
  ...
  for (ch = combat_list; ch; ch = next_combat_list) {
    ...
    if (ch->in_room && world[ch->in_room].poltergeist[0]) {
      ...
      if (dam > 0) {
        send_to_char("You are hit by flying objects!\r\n", ch);
        damage(ch, ch, dam, 0, MENTAL);
      }
    }
    roll_individual_initiative(ch);
  }
