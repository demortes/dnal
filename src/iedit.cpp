...
#define NUM_CYBERWARE_TYPES     35
#define NUM_BIOWARE_TYPES        12
...
void iedit_disp_val2_menu(struct descriptor_data * d)
{
  ...
  case ITEM_MONEY:
    ...
    break;
  case ITEM_MOD:
    if (GET_OBJ_VAL(d->edit_obj, 0) == MOD_MOUNT)
...
void iedit_disp_val10_menu(struct descriptor_data * d)
{
  ...
  {
  case ITEM_WEAPON:
    if (access_level(CH, LVL_ADMIN) && GET_OBJ_VAL(OBJ, 9) >= -1)
...
void iedit_disp_menu(struct descriptor_data * d)
{
  ...
  OBJ->obj_flags.bitvector.PrintBits(buf1, MAX_STRING_LENGTH,
                                     affected_bits, AFF_MAX);
  send_to_char(CH, "7) Item affection flags: %s%s%s\r\n", CCCYN(CH, C_CMP), buf1,
               CCNRM(CH, C_CMP));
  ...
  send_to_char(CH, "d) Item Material: %s%s%s\r\n", CCCYN(CH, C_CMP),
               material_names[(int)GET_OBJ_MATERIAL(d->edit_obj)], CCNRM(CH, C_CMP));
...
void iedit_parse(struct descriptor_data * d, char *arg)
{
  ...
  case IEDIT_VALUE_1:
    ...
    case ITEM_CYBERWARE:
      if (number < 1 || number > 10) {
        send_to_char("Value must be between 1 and 10.\r\nRating of cyberware: ", CH);
        return;
      }
      break;
    case ITEM_CYBERDECK:
  ...
  case IEDIT_VALUE_3:
    ...
    case ITEM_DECK_ACCESSORY:
      ...
      break;
    case ITEM_GUN_ACCESSORY:
      if (GET_OBJ_VAL(OBJ, 1) == 3 && (number < -3 || number > -1)) {
