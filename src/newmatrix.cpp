...
ACMD(do_matrix_score)
{
  ...
  sprintf(buf, "You are connected to the matrix.\r\n"
          ...
          detect, MAX(0, GET_REM_HACKING(ch)), GET_HACKING(ch),
...
ACMD(do_create)
{
  ...
  if (is_abbrev(buf1, "program")) {
    ...
    if (GET_SKILL(ch, SKILL_BR_COMPUTER) < 1)  {
    ...
    send_to_char("You can only create parts or programs spells.\r\n", ch);
    return;
  }
}
...
ACMD(do_asist)
{
  if (!PERSONA)
    ...
    send_to_char(ch, "You can't switch ASIST modes with a cold ASIST interface.\r\n");
  else if (DECKER->asist[0]) {
    DECKER->asist[0] = 0;
    send_to_char(ch, "You switch to cold ASIST mode.\r\n");
    GET_MAX_HACKING(ch) = 0;
    DECKER->response = 0;
  } else {
    send_to_char(ch, "You switch to hot ASIST mode.\r\n");
    DECKER->asist[0] = 1;
