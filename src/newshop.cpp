                  ...
                  struct shop_sell_data *sell, struct obj_data *obj, struct obj_data *cred, int price, vnum_t shop_nr)
{
    ...
    if (GET_OBJ_TYPE(obj) == ITEM_CYBERWARE) {
      ...
      for (check = ch->cyberware; check != NULL; check = check->next_content) {
        ...
        if (GET_OBJ_VAL(check, 2) == GET_OBJ_VAL(obj, 2)) {
          printf(ENDOF(buf), " You already have a similar piece of cyberware.", GET_CHAR_NAME(ch));
          do_say(keeper, buf, cmd_say, SCMD_SAYTO);
          return FALSE;
        }
      }
      if (GET_OBJ_VAL(obj, 2) == 23 || GET_OBJ_VAL(obj, 2) == 30 || GET_OBJ_VAL(obj, 2) == 20)
        for (check = ch->bioware; check; check = check->next_content) {
          if (GET_OBJ_VAL(check, 2) == 2 && GET_OBJ_VAL(obj, 2) == 23) {
            sprintf(ENDOF(buf), " %s is not compatible with orthoskin.", obj->text.name);
            do_say(keeper, buf, cmd_say, SCMD_SAYTO);
            return FALSE;
          }
          if (GET_OBJ_VAL(check, 2) == 8 && GET_OBJ_VAL(obj, 2) == 30) {
            sprintf(ENDOF(buf), " %s is not compatible with synaptic accelerators.", obj->text.name);
            do_say(keeper, buf, cmd_say, SCMD_SAYTO);
            return FALSE;
          }
          if (GET_OBJ_VAL(check, 2) == 10 && GET_OBJ_VAL(obj, 2) == 20) {
            sprintf(ENDOF(buf), " %s is not compatible with muscle augmentation.", obj->text.name);
            do_say(keeper, buf, cmd_say, SCMD_SAYTO);
            return FALSE;
          }
        }
      ...
      if (GET_OBJ_VAL(obj, 2) == 2 || GET_OBJ_VAL(obj, 2) == 8 || GET_OBJ_VAL(obj, 2) == 10)
        for (check = ch->cyberware; check; check = check->next_content) {
          if (GET_OBJ_VAL(check, 2) == 23 && GET_OBJ_VAL(obj, 2) == 2) {
            sprintf(ENDOF(buf), " Orthoskin is not compatible with any form of dermal plating.");
            do_say(keeper, buf, cmd_say, SCMD_SAYTO);
            return FALSE;
          }
          if (GET_OBJ_VAL(check, 2) == 20 && GET_OBJ_VAL(obj, 2) == 10) {
            sprintf(ENDOF(buf), " Muscle augmentation is not compatible with muscle replacement.");
            do_say(keeper, buf, cmd_say, SCMD_SAYTO);
            return FALSE;
          }
        }
  ...
  } else
  {
    ...
    if ((GET_OBJ_TYPE(obj) == ITEM_DECK_ACCESSORY && GET_OBJ_VAL(obj, 0) == TYPE_PARTS) ||
      ...
      GET_OBJ_COST(obj) = GET_OBJ_COST(obj) * buynum;
  ...
  if (bought > 1)
  {
    sprintf(buf, " (x%d)", bought);
    sprintf(arg, buf2, buf);
  } else
    sprintf(arg, buf2, "");
  send_to_char(arg, ch);
  return TRUE;
...
void shop_buy(char *arg, struct char_data *ch, struct char_data *keeper, vnum_t shop_nr)
{
  ...
  {
    if (sell->lastidnum == GET_IDNUM(ch)) {
    ...
    if (success < 1 || sell->lastidnum == GET_IDNUM(ch)) {
      sprintf(buf, "%s I can't get ahold of that one for a while.", GET_CHAR_NAME(ch));
      do_say(keeper, buf, cmd_say, SCMD_SAYTO);
      sell->lastidnum = GET_IDNUM(ch);
    } else {
      float totaltime = ((GET_OBJ_AVAILDAY(obj) * buynum) / success);
...
void shop_sell(char *arg, struct char_data *ch, struct char_data *keeper, vnum_t shop_nr)
{
  ...
  if (!sell)
  {
    if (!shop_table[shop_nr].flags.IsSet(SHOP_NORESELL)) {
...
void shop_list(char *arg, struct char_data *ch, struct char_data *keeper, vnum_t shop_nr)
{
  ...
  if (shop_table[shop_nr].flags.IsSet(SHOP_DOCTOR))
  {
    ...
    for (struct shop_sell_data *sell = shop_table[shop_nr].selling; sell; sell = sell->next, i++) {
      ...
      sprintf(ENDOF(buf), "%-33s^n %-6s%2d   %0.2f%c  %9d\r\n", GET_OBJ_NAME(obj),
              GET_OBJ_TYPE(obj) == ITEM_CYBERWARE ? "Cyber" : "Bio", GET_OBJ_VAL(obj, 0), ((float)GET_OBJ_VAL(obj, 1) / 100),
              GET_OBJ_TYPE(obj) == ITEM_CYBERWARE ? 'E' : 'I', buy_price(obj, shop_nr));
      extract_obj(obj);
    }
  } else
  {
    sprintf(buf, " ##   Available   Item                                              Price\r\n"
            "--------------------------------------------------------------------------\r\n");
    for (struct shop_sell_data *sell = shop_table[shop_nr].selling; sell; sell = sell->next, i++) {
      obj = read_object(sell->vnum, VIRTUAL);
      if (!obj || GET_OBJ_COST(obj) < 1) {
...
void shop_info(char *arg, struct char_data *ch, struct char_data *keeper, vnum_t shop_nr)
{
  ...
  sprintf(ENDOF(buf), " I couldn't let it go for less than %d nuyen.", buy_price(obj, shop_nr));
  do_say(keeper, buf, cmd_say, SCMD_SAYTO);
}
  ...
  for (counter = zone_table[zone].number * 100;
    ...
    if (realcounter >= 0) {
      ...
      fprintf(fp, "NoSuchItemKeeper:\t%s\n"
              ...
              "Buy:\t%s\n"
              "Sell:\t%s\n",
              shop->no_such_itemk, shop->no_such_itemp, shop->not_enough_nuyen, shop->doesnt_buy, shop->buy, shop->sell);
      fprintf(fp, "Flags:\t%s\n"
              "Races:\t%s\n"
              "Buytypes:\t%s\n"
              "Etiquette:\t%d\n",
...
{
  CLS(CH);
  send_to_char(CH, "1) No Such Item (Keeper): ^c%s^n\r\n", SHOP->no_such_itemk);
  send_to_char(CH, "2) No Such Item (Player): ^c%s^n\r\n", SHOP->no_such_itemp);
  send_to_char(CH, "3) Not Enough Nuyen: ^c%s^n\r\n", SHOP->not_enough_nuyen);
  send_to_char(CH, "4) Doesn't Buy: ^c%s^n\r\n", SHOP->doesnt_buy);
  send_to_char(CH, "5) Buy: ^c%s^n\r\n", SHOP->buy);
  send_to_char(CH, "6) Sell: ^c%s^n\r\n", SHOP->sell);
