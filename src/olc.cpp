...
ACMD(do_medit)
{
  ...
  if (mob_num >= 0) {
    ...
    *mob = mob_proto[mob_num]; // the RNUM
    ...
    if (mob_proto[mob_num].mob_specials.arrive)
      mob->mob_specials.arrive = str_dup(mob_proto[mob_num].mob_specials.arrive);
    if (mob_proto[mob_num].mob_specials.leave)
      mob->mob_specials.leave = str_dup(mob_proto[mob_num].mob_specials.leave);
    ...
    d->edit_mob->mob_specials.arrive = str_dup("arrives from");
    d->edit_mob->mob_specials.leave = str_dup("leaves");
...
ACMD(do_shedit)
{
  ...
  if (real_shop(number) > -1) {
    ...
    d->edit_shop->sell = str_dup("Sorry, we're closed.");
