...
void check_quest_delivery(struct char_data *ch, struct char_data *mob, struct obj_data *obj)
{
...
bool rep_too_high(struct char_data *ch, int num)
{
  ...
  {
    for (f = ch->followers; f; f = f->next)
      if (GET_REP(f->follower) > quest_table[num].max_rep)
        return TRUE;
  }
...
bool rep_too_low(struct char_data *ch, int num)
{
  ...
  {
    for (f = ch->followers; f; f = f->next)
      if (GET_REP(f->follower) < quest_table[num].min_rep)
        return TRUE;
  }
...
SPECIAL(johnson)
{
  ...
  if (CMD_IS("say") || CMD_IS("'")) {
    if (str_str(argument, "quit"))
      comm = CMD_QUIT;
    else if (str_str(argument, "collect") || str_str(argument, "complete"))
      comm = CMD_DONE;
      ...
      comm = CMD_START;
    else if (str_str(argument, "yes") || str_str(argument, "accept"))
      comm = CMD_YES;
  ...
  if (comm == CMD_QUIT && GET_SPARE1(johnson) == -1 && GET_QUEST(ch) &&
    ...
    if (obj_complete || mob_complete) {
      ...
      do_say(johnson, "But you have not completed any objectives yet.", 0, 0);
             ...
             !GET_QUEST(ch) && GET_SPARE2(johnson) >= 0) {
    for (int i = QUEST_TIMER - 1; i >= 0; i--)
