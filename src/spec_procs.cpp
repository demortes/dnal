...
SPECIAL(teacher)
{
  ...
  if (!*argument) {
    ...
    for (int i = 0; i < 8; i++) {
      if (teachers[ind].s[i] > 0) {
        ...
        else if (GET_SKILL(ch, teachers[ind].s[i]) < maxi && !ch->char_specials.saved.skills[teachers[ind].s[i]][1])
  ...
  if (GET_SKILL(ch, skill_num) >= maxi) {
...
SPECIAL(adept_trainer)
{
  ...
  switch (power) {
  case ADEPT_IMPROVED_BOD:
    if (GET_REAL_BOD(ch) + GET_POWER(ch, ADEPT_IMPROVED_BOD) >= racial_limits[(int)GET_RACE(ch)][0][0] - GET_PERM_BOD_LOSS(ch))
      cost *= 2;
    break;
  case ADEPT_IMPROVED_QUI:
...
SPECIAL(orkish_truckdriver)
{
  ...
  if (FIGHTING(driver)) {
    if (quest_table[GET_QUEST(FIGHTING(driver))].vnum == 5000) {
      ...
      if (world[driver->in_room].dir_option[WEST]) {
        ...
        delete world[real_room(5062)].dir_option[EAST];
        world[real_room(5062)].dir_option[EAST] = NULL;
  ...
  if (world[driver->in_room].number == 8198 && (time_info.hours < 2 || time_info.hours > 3)) {
    ...
    if (!world[driver->in_room].dir_option[WEST]) {
      ...
      world[real_room(5062)].dir_option[EAST] = new room_direction_data;
      world[real_room(5062)].dir_option[EAST]->to_room = driver->in_room;
    }
