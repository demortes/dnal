...
#define IS_ELEMENTAL(ch) (IS_NPC(ch) && GET_RACE(ch) == CLASS_ELEMENTAL)
...
#define STUNNED(ch)           (GET_POS(ch) <= POS_STUNNED && GET_QUI(ch) > 0)
...
#define IS_WORKING(ch)        ((AFF_FLAGS(ch).AreAnySet(AFF_PROGRAM, AFF_DESIGN, AFF_PART_BUILD, AFF_PART_DESIGN, AFF_PILOT, AFF_RIG, AFF_BONDING, AFF_CONJURE, AFF_PACKING, AFF_LODGE, AFF_CIRCLE, ENDBIT)))
#define STOP_WORKING(ch)      {AFF_FLAGS((ch)).RemoveBits(AFF_PROGRAM, AFF_DESIGN, AFF_PART_BUILD, AFF_PART_DESIGN, AFF_BONDING, AFF_CONJURE, AFF_PACKING, AFF_LODGE, AFF_CIRCLE, ENDBIT); \
...
#define IS_GUN(type) (((type) >= TYPE_PISTOL) && ((type) < TYPE_SUFFERING) && (type) != TYPE_TASER)
