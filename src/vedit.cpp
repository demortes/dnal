...
void vedit_disp_menu(struct descriptor_data * d)
{
  ...
  send_to_char(CH, "5) Vehicle Inside Descript: \r\n%s\r\n",
               ...
               "(not set)");
  send_to_char(CH, "6) Arrive text: ^c%s^n,  7) Leave text: ^c%s^n\r\n",
               d->edit_veh->arrive, d->edit_veh->leave);
  d->edit_veh->flags.PrintBits(buf1, MAX_STRING_LENGTH, veh_flag, NUM_VFLAGS);
  send_to_char(CH, "8) Flags: ^c%s^n\r\n", buf1);
  send_to_char(CH, "9) H(^c%d^n) Sp(^c%d^n) Acc(^c%d^n) B(^c%d^n) Ar(^c%d^n)\r\n",
               d->edit_veh->handling, d->edit_veh->speed, d->edit_veh->accel, d->edit_veh->body,
               d->edit_veh->armor);
  send_to_char(CH, "   Sig(^c%d^n) Au(^c%d^n) P(^c%d^n) Seat(^c%d^n) Load(^c%d^n) Cost(^c%d^n)\r\n",
               d->edit_veh->sig, d->edit_veh->autonav, d->edit_veh->pilot, d->edit_veh->seating,
               (int)d->edit_veh->load, d->edit_veh->cost);
  send_to_char(CH, "0) Type: ^C%s^n\r\n", veh_type[d->edit_veh->type]);
...
void vedit_disp_att(struct descriptor_data * d)
{
  ...
  send_to_char(CH, "9) Seating:  ^c%d^n\r\n", d->edit_veh->seating);
...
void vedit_parse(struct descriptor_data * d, char *arg)
{
  ...
  case VEDIT_ATT:
    ...
    case '8':
      ...
      break;
    case '9':
      send_to_char(CH, "Enter Seating Attribute: ");
  ...
  case VEDIT_SEAT:
    ...
    if ((number < 0) || (number > 50)) {
      send_to_char("Value must range between 0 and 50.\r\n", CH);
      send_to_char("Enter Seating attribute: ", CH);
    } else {
      d->edit_veh->seating = number;
      vedit_disp_att(d);
    }
    break;
  case VEDIT_COST:
    ...
    if (realcounter >= 0) {
      *veh = veh_proto[realcounter];
      ...
      fprintf(fp, "Sig:\t%d\n"
              ...
              "Type:\t%d\n"
              "Flags:\t%s\n",
              veh->sig, veh->autonav, veh->seating, (int)veh->load, veh->cost, veh->type,
              veh->flags.ToString());
      fprintf(fp, "BREAK\n");
    }
  }
